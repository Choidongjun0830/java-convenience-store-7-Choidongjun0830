...
## 기능 구현
1. products.md, promotions.md 파일 읽기
2. 읽은 결과를 바탕으로 Repository에 업데이트
3. 상품 주문 받기 - 파싱으로 주문 상품과 주문 수량 정리
4. 프로모션과 주문 상품 연결
5. 프로모션 적용한 구매 - 프로모션 적용 가능한테 주문 수량 부족 시 안내
6. 프로모션 적용 후 남은 주문 수량 안내 후 구매
7. 프로모션 미적용 금액에 멤버십 적용
8. 영수증 출력

## 고려한 예외 상황
1. 재고에 없는 상품 주문
2. 프로모션 진행 기간이 아닐 때, 일반 재고만 고려하여 재고 초과
3. 프로모션 진행 기간일 때, 프로모션 + 일반 재고 고려하여 재고 초과
4. 입력 형식에 맞지 않는 입력 - 공백, 타입, 포맷 
5. 상품 수 0이나 음수로 주문 시
6. [사이다-3],[사이다-15]처럼 주문했을 시, 재고 초과
